...
from numpy import reshape, mean, array, unique, concatenate, where
...
class DbscanSklearn(RootCluster):
    ...
    def _cluster__(self, X_data=None):
        ...
        t1_uni_label, t2_sum_label = unique(labels, return_counts=True)
        centers = []
        for label in t1_uni_label:
            temp = X_data[ where(labels == label) ]
            center = mean(temp, axis=0)
            centers.append(center)
        if t1_uni_label[0] == -1:
            centers = array(centers)[1:]
