...
from numpy import reshape, argmin, min, concatenate, zeros, append, sort, argsort, exp, ceil
...
class ImmuneInspiration(RootCluster):
    ...
    def _cluster__(self, X_data=None):
        ...
        y_pred = zeros(len(X_data))
        hu1 = [0, deepcopy(X_data[randint(0, len(X_data))])]  # hidden unit 1 (t1, wH)
        list_clusters = [deepcopy(hu1)]                         # list hidden units
        centers = deepcopy(hu1[1]).reshape(1, hu1[1].shape[0])  # 2-d array
        m = 0
        while m < len(X_data):
            D = cdist(reshape(X_data[m], (1, -1)), centers)  # calculate pairwise distances btw mth point and centers
            c = argmin(D, axis=1)[0]        # return index of the closest center
            distmc = min(D, axis=1)      # distmc: minimum distance
            ...
            if distmc < self.stimulation_level:
                ...
                centers = append(centers, [deepcopy(X_data[m])], axis=0)
        ...
        self.feature_label = concatenate((X_data, reshape(y_pred, (-1, 1))), axis=1)
    ...
    def _cluster__(self, X_data=None):
        ...
        while m < len(X_data):
            ...
            if distmc < self.stimulation_level:
                ...
                # Find Neighbourhood
                nei = cdist(reshape(centers[c], (1, -1)), centers)
                nei_id_sorted = argsort(nei)
                nei_dist_sorted = sort(nei)
